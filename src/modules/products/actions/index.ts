...
const validationUpsertProduct = (data: Record<string, any>) => {
    const formSchema = z.object({
        ...
        price: z.float32({ message: "price must be float" }).min(1, { message: "price must be at least 1" }),
        quantity: z.number({ message: "quantity must be number" }).min(1, { message: "quantity must be at least 1" }).max(1000, { message: "quantity must be at max 1000" }),
...
export const upsertProduct = async (prevData: { data: Product | null, error: Record<string, string> | null }, formData: FormData) => {
